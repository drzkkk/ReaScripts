-- @description Select MIDI note to below of current selection
-- @version 1.1
-- @tags midi, notes, select
-- @changelog Added logic to select highest note if none selected
-- @author drzk
...
if not current_note then
  ...
  local _, _, _, _, _, _, highest_pitch = r.MIDI_GetNote(take, 0)

  for i = 1, note_count - 1 do
    local _, _, _, _, _, _, pitch = r.MIDI_GetNote(take, i)
    if pitch > highest_pitch then
      highest_index = i
      highest_pitch = pitch
    end
  end
  ...
  local ret, _, mut, sppq, eppq, ch, pit, vel = r.MIDI_GetNote(take, highest_index)
  r.MIDI_SetNote(take, highest_index, true, mut, sppq, eppq, ch, pit, vel, false)
...
local function distance(cand)
  local dp = current_note.pitch - cand.pitch
