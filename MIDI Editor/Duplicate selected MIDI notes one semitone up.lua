-- @description Duplicate selected MIDI notes one semitone up
-- @version 1.0
...
local function find_free_pitch_up(desired_pitch, chan, startppq, endppq)
  local test_pitch = desired_pitch
  while test_pitch <= 127 do
...
for _, note in ipairs(selected_notes) do
  local desired_pitch = note.pitch + 1
  if desired_pitch <= 127 then
    local free_pitch = find_free_pitch_up(desired_pitch, note.chan, note.startppq, note.endppq)
    if free_pitch then
...
reaper.Undo_EndBlock("Duplicate selected MIDI notes one semitone up", -1)