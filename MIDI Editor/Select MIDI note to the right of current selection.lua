...
-- @changelog Added logic to select earliest note if none is selected
-- @author drzk
...
if not current_note then
  local first_index = 0
  local _, _, _, first_ppq, _, _, first_pitch = r.MIDI_GetNote(take, 0)

  for i = 1, note_count - 1 do
    local _, _, _, ppq, _, _, _ = r.MIDI_GetNote(take, i)
  ...
  local ret, _, mut, sppq, eppq, ch, pit, vel = r.MIDI_GetNote(take, first_index)
  r.MIDI_SetNote(take, first_index, true, mut, sppq, eppq, ch, pit, vel, false)
...
local ret, _, mut, sppq, eppq, ch, pit, vel = r.MIDI_GetNote(take, target.index)
