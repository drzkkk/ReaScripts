-- @description Duplicate track above with FX and automation, copying only selected or Razor Edit items
-- @version 1.0
...
local function DuplicateTrackAbove(src_track)
  local src_idx = r.GetMediaTrackInfo_Value(src_track, "IP_TRACKNUMBER") - 1
  r.InsertTrackAtIndex(src_idx, true)
  local dst_track = r.GetTrack(0, src_idx)
...
function Main()
  ...
  local dst_track = DuplicateTrackAbove(src_track)
  ...
  r.UpdateArrange()
  r.Undo_EndBlock("Duplicate track above with FX and automation", -1)
end
