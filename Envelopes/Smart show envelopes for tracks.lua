-- @description Smart show envelopes for tracks if no envelopes show Volume envelope (using Utility jsfx)
-- @version 1.0
-- @tags envelopes
...
local function has_any_envelope(track)
    local env_count = r.CountTrackEnvelopes(track)
    return env_count > 0
end

if has_any_envelope(tr) then
...
if fx_idx == -1 then
    ...
    local env = r.GetFXEnvelope(tr, fx_idx, 0, true)
